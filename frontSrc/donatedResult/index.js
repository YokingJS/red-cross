...
class Page extends React.Component {
  ...
  componentWillMount() {
    ...
    if(params.orderid) {
      ...
      let donateInfoListStr = window.getCookie('jiushu_data_donateInfoList'); 
      let donateInfoList = JSON.parse(donateInfoListStr);
      this.setState({
        isFromSearch: true,
        donateInfoList: donateInfoList
      });
    }
  }
  ...
  renderDataList() {
    ...
    if (!isFromSearch) {
      alternativeTitle = window.getCookie('jiushu_data_appealRecordTitle');
    }
    return dataList.map((item, index) => {
      const {
        appealRecordId = '', money = '', donateType = 0, gmtModify = '', appealRecordTitle = ''
      } = item;
      return [
